...
function check(hash, block) {
  tronWeb.trx.getBalance(tronWeb.defaultAddress.hex).then((err, balance) => {
    console.log('Balance before check', balance / 1000000);
  });
  tronWeb.trx.getBandwidth(tronWeb.defaultAddress.hex).then((err, bandwidth) => {
    console.log('Bandwidth before check', bandwidth);
  });
  ...
  factoryContract.checkBlockNumber(`0x${hash}`).send({
    ...
    tronWeb.trx.getBalance(tronWeb.defaultAddress.hex).then((err, balance) => {
      console.log('Balance after check', balance / 1000000);
    });
    tronWeb.trx.getBandwidth(tronWeb.defaultAddress.hex).then((err, bandwidth) => {
      console.log('Bandwidth after check', bandwidth);
    });
  });
}
    ...
    if (result.newStage === '5') {
      factoryContract.getStatus(`0x${result.hash}`).call().then((data) => {
        update(
          ...
          data.revealCount).catch(e => console.error(e));
      }).catch(e => {
        console.error(e);
      });
    } else {
